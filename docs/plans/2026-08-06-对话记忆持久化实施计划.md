...
**Spec:** `docs/对话记忆持久化设计文档.md`（Draft v3，2026-08-07 终审定稿）

**2026-08-07 终审修订**（对照代码逐项核实 + 三项决策确认后纳入，明细见文末 Self-Review 记录）：修复 sse.ts 包装层吞 onDone（P1）、CORS 补 PATCH/DELETE（P1）、planner.md 补【会话记忆】指引（P1）、历史消息注入前逐条清洗 + 截断（安全决策）、后台任务登记防 GC、新测试文件补 SKIP 防护、isStreaming 提前置位、侧栏断点统一 768px、ChatWidget 加 30 天留存隐私提示（产品决策）、.gitignore 补 WAL 副产物、模块 README 同步。
...
- 历史消息**落库存原文、注入前逐条清洗**：`build_history_text` / 摘要输入对每条 content 过 `agent/sanitize.py::sanitize_history`（非抛出式，命中注入模式整条替换为 `[历史内容已过滤]`，不剥代码块），并截断至 500 字
- 后端 fire-and-forget 后台任务（摘要/标题/兜底落库）一律经 `_spawn()` 登记到模块级 task set，防事件循环弱引用被 GC 提前回收
...
- `agent/sanitize.py` — 新增 `sanitize_history()`（非抛出式注入筛查）
- `prompts/planner.md` — 新增【会话记忆】小节（v1.2.0 → v1.3.0）
...
- `frontend/src/components/chat/ChatWidget.vue` — 输入区下方加"30 天留存"隐私提示

**部署/文档：**
- `deploy/xiehaoyu-agent.service` — ReadWritePaths 加 `data/`
- `.env.example`、`README.md` 配置表
- `.gitignore` — 补 `*.db-wal` / `*.db-shm`（WAL 副产物）
- `prompts/README.md`、`backend/README.md`、`agent/README.md` — 同步新 prompt / 接口 / 签名
...
**已知接受项（不在本期修）：** 前端手动重试会在同一会话产生重复 user 消息（重试 = 新的一轮），属诚实展示，不特殊处理；sse 自动重试若恰好落在"服务端已落库但客户端未收到 [DONE]"的极端窗口，同样会产生重复轮次（低概率，同类不处理）。`/api/sessions/*` 不加限流（有 50 个/用户 + 30 天清理兜底）。`X-Session-Id` 响应头不做跨域 expose（仅兼容路径使用，新前端走 POST /api/sessions 的 JSON body 拿 session_id）。
...
(c) CORS 两处修改——`allow_headers` 放行 `X-User-Id`；`allow_methods` 补 `PATCH`/`DELETE`
（当前仅 `GET`/`POST`，重命名/删除在跨域预检会被拦，与 X-User-Id 属同一类加固，终审修订）：
```python
...
- Modify: `prompts/planner.md`（新增【会话记忆】小节，v1.2.0 → v1.3.0）
...
- [ ] **Step 4: planner.md 新增【会话记忆】小节（v1.3.0）**

四个工具只收到 planner 传的 `args.question`，自身看不到历史——"那 2017 年呢"这类
追问的指代消解必须由 planner 显式完成，不能靠模型自行领悟（终审修订 P1）。
在 `prompts/planner.md` 的【决策规则】之后、【安全规则（最高优先级）】之前插入：
...
版本头改为 `<!-- version: 1.3.0, date: 2026-08-07 -->`。

- [ ] **Step 5: 实现 graph 接线 + steps 修复**
...
- [ ] **Step 6: 跑测试确认通过（含 graph/planner 既有测试）**
...
```bash
git add agent/planner.py agent/graph.py prompts/planner.md tests/test_memory_injection.py
git commit -m "feat(memory): planner 记忆注入（history_text 独立 user 消息）+ planner.md 会话记忆小节 + 修复 final_answer.steps 恒 0"
```
...
- Modify: `agent/sanitize.py`（新增 `sanitize_history`，非抛出式）
- Test: `tests/test_summarizer.py`

**Interfaces:**
- Consumes: Task 1 配置、Task 2 store、`agent.llm_client.get_client`、`agent.sanitize.sanitize_history`
- Produces:
...
- [ ] **Step 4: sanitize.py 新增 sanitize_history（非抛出式，终审修订）**

`agent/sanitize.py` 末尾追加：
...
```bash
git add prompts/summary.md backend/app/services/summarizer.py agent/sanitize.py tests/test_summarizer.py
git commit -m "feat(memory): 触发式摘要 + 标题自动生成（per-session 并发防护）+ 历史注入筛查/截断"
```
...
(b) `sendMessage` 开头（`if (isStreaming.value) return` 之后）先提前置位 streaming、再插入幂等建会话：
```ts
...
- Modify: `frontend/src/components/chat/ChatWidget.vue`（输入区下方加"30 天留存"隐私提示）
...
```
新增 `@media (max-width: 767px)` 媒体查询块（终审修订：与 JS 折叠判断的 768px 断点对齐；
原计划复用 640px 块，会在 640–768px 区间留下侧栏内联挤压布局的问题）：
```css
...
- [ ] **Step 3: ChatWidget 加访客隐私提示（终审产品决策）**

公网访客的对话会在服务端保存 `MEMORY_MAX_AGE_DAYS=30` 天，需对访客诚实告知。
`frontend/src/components/chat/ChatWidget.vue` 的 `.input-area` 内、`<ChatInput>` 之后加一行：
...
7. 输入框下方常驻"对话记录仅保存 30 天"隐私提示小字

- [ ] **Step 6: Commit**

```bash
git add frontend/src/components/chat/SessionSidebar.vue frontend/src/views/ChatView.vue frontend/src/components/chat/ChatWidget.vue
git commit -m "feat(memory): 会话侧栏（列表/搜索/重命名/删除/新建）+ 响应式折叠 + 隐私提示"
```
...
- Modify: `.gitignore`（WAL 副产物）
- Modify: `prompts/README.md`、`backend/README.md`、`agent/README.md`（同步新 prompt / 接口 / 签名）
...
- [ ] **Step 2: .gitignore 补 WAL 副产物**

WAL 模式运行期会产生 `sessions.db-wal` / `sessions.db-shm`，现有 `*.db` 规则覆盖不到。
`.gitignore` 的 `*.db` 行之后加：
...
- [ ] **Step 4: 模块 README 同步（仓规：改动同步文档）**

- `prompts/README.md`：文件清单加 `summary.md` 行（v1.0.0，会话摘要，`summarizer.py`）；解析标记表加一行（`summary.md`：`【系统角色】` `【历史摘要】`，两者之间 → system，其余 → user 模板）；占位符列表加 `{old_summary}` `{new_dialogue}`；`planner.md` 行版本号改 1.3.0
- `backend/README.md`：API 表加 `/api/sessions` 系列 6 个端点与 `X-User-Id` 身份头说明
- `agent/README.md`：`stream_run` / `run` 签名补 `history_text` 参数说明

- [ ] **Step 5: Commit**

```bash
git add deploy/xiehaoyu-agent.service README.md .gitignore prompts/README.md backend/README.md agent/README.md
git commit -m "feat(memory): systemd 放行 data/ + 配置表/gitignore/模块 README 同步"
```
...
**2026-08-07 终审修订**（对照代码逐项核实 + 三项决策确认后纳入）：
- Task 9：修复 `sseChatStream` 包装层吞掉调用方 `onDone` 的既有隐患（P1，Task 10 的列表刷新依赖它）——改为链式调用
- Task 4：CORS `allow_methods` 补 PATCH/DELETE（P1，与 X-User-Id 同类加固）；测试计数修正 14→13
- Task 5：planner.md 新增【会话记忆】小节（P1：工具自身看不到历史，追问指代消解必须显式指引 planner）
- Task 6：历史消息注入前逐条 `sanitize_history` + 截断 500 字（安全决策：注入原文不随历史回放进 planner；落库仍存原文）
- Task 7：后台任务经 `_spawn()` 登记 task set 防 GC；新测试文件补 `SKIP_CONFIG_VALIDATION` 模块级防护
- Task 10：`isStreaming` 提前置位，防 `ensureSession` 重入双建会话
- Task 11：侧栏浮层断点 640px → 767px 与 JS 的 768px 对齐；ChatWidget 加 30 天留存隐私提示（产品决策）
- Task 12：`.gitignore` 补 WAL 副产物；prompts/backend/agent README 同步
