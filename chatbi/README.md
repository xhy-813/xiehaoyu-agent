...
| [few_shots.py](few_shots.py) | 7 组（问题, SQL）few-shot 示例，`format_few_shots()` 格式化注入 prompt |
...
- 只允许 `SELECT` 或 `WITH ... SELECT`（CTE 会被 sqlparse 报为 UNKNOWN，单独识别放行）。
...
覆盖：时间过滤+聚合、分组统计、JOIN+排序 top-N、多表 JOIN+去重计数、条件聚合、`COUNT(*)` vs `COUNT(DISTINCT)` 语义、多年长表对比。在 [few_shots.py](few_shots.py) 中增改。
