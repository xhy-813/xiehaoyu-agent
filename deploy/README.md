...
- **域名与 HTTPS（2026-08-15 已启用）**：`xiehaoyu.cn`（证书 SAN 含 www）使用腾讯云 TrustAsia 免费 DV 证书——**2026-11-12 到期，无自动续期**：到期前从腾讯云 SSL 控制台重新下载 Nginx 格式证书包，替换 `/etc/ssl/certs/xiehaoyu.cn_bundle.crt` 与 `/etc/ssl/private/xiehaoyu.cn.key` 后 `sudo systemctl reload nginx`。线上配置为 443 server 块 + 80 全量 301 跳转（裸 IP 访问同样跳转规范域名）。
